...
import com.open.taogubaweex.utils.ScreenUtil;
 ...
 */
public class WeeXHtmlTextDomObject extends WXDomObject {
	...
	static final CSSNode.MeasureFunction TEXT_MEASURE_FUNCTION = new CSSNode.MeasureFunction() {
		...
		public void measure(CSSNode node, float width, @NonNull MeasureOutput measureOutput) {
			WeeXHtmlTextDomObject textDomObject = (WeeXHtmlTextDomObject) node;
	...
	@Override
	public WeeXHtmlTextDomObject clone() {
		WeeXHtmlTextDomObject dom = null;
		try {
			dom = new WeeXHtmlTextDomObject();
			copyFields(dom);
	...
	protected void updateSpannable(Spannable spannable, int spanFlag) {
		...
		if (mFontSize == UNSET) {
			ops.add(new SetSpanOperation(0, spannable.length(), new AbsoluteSizeSpan((int) ScreenUtil.dpToPxInt(getDomContext().getUIContext(), 18f)), spanFlag));
		}
